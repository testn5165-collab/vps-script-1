...
- **SSH-WS** — SSH tunneled over WebSocket (HTTP upgrade) via a **Python proxy** (`ws-proxy.py`), CDN/proxy-friendly
...
- A single HAProxy instance — 16 public ports (9 TLS + 7 plain), all sharing the same **Python-based SSH-WS proxy engine** (`ws-proxy.py`) + V2Ray backend
- Python WS-proxy handles the WebSocket handshake and forwards traffic to Dropbear — this is the same engine behind every SSH-WS and SSH-WS+SSL port listed above
...
| `04-ssh-ws.sh` | SSH-WebSocket engine — installs `/usr/local/bin/ws-proxy.py` (Python), listens on `127.0.0.1:12082`, forwards to Dropbear |
